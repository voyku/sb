...
green "域名证书申请成功或已存在！域名证书（cert.crt）和密钥（private.key）已保存到 /root/cert文件夹内" 
yellow "公钥文件crt路径如下，可直接复制"
green "/root/cert/cert.crt"
yellow "密钥文件key路径如下，可直接复制"
green "/root/cert/private.key"
echo $ym > /root/cert/ca.log
if [[ -f '/etc/hysteria/config.json' ]]; then
blue "检测到Hysteria-1代理协议，如果你安装的是甬哥的Hysteria-yg脚本，此证书将自动应用"
fi
if [[ -f '/etc/caddy/Caddyfile' ]]; then
blue "检测到Naiveproxy代理协议，如果你安装的是甬哥的Naiveproxy-yg脚本，此证书将自动应用"
fi
if [[ -f '/etc/tuic/tuic.json' ]]; then
blue "检测到Tuic代理协议，如果你安装的是甬哥的Tuic-yg脚本，此证书将自动应用"
fi
if [[ -f '/usr/bin/x-ui' ]]; then
blue "检测到x-ui（xray代理协议），如果你安装的是甬哥的x-ui-yg脚本,此证书将自动应用"
fi
if [[ -f '/etc/s-box/sb.json' ]]; then
blue "检测到Sing-box，如果你安装的是甬哥的Sing-box-yg脚本,此证书将自动应用"
fi
else
fail
fi
...
yellow " 二、独立80端口模式仅支持单域名证书申请，在80端口不被占用的情况下支持自动续期"
yellow " 三、DNS API模式不支持freenom免费域名申请，支持单域名与泛域名证书申请，无条件自动续期"
yellow " 四、泛域名申请前须要在解析平上设置一个名称为 * 字符的解析记录"
yellow " 公钥文件crt保存路径：/root/cert/cert.crt"
yellow " 密钥文件key保存路径：/root/cert/private.key"
echo
echo "#############################################################"
acmeshow
